...
#     # =======================================
#     #  DELETE ONLY MATCHING ROWS (UPSERT)
#     # =======================================
#     with engine.begin() as conn:
...
def write_forecast_to_db(df: pd.DataFrame, prediction_type: str):
    ...
    # UPSERT Logic: delete matching rows before inserting
    with engine.begin() as conn:
        for _, row in df.iterrows():
            conn.execute(text(f"""
                DELETE FROM {schema}.{table}
    ...
    logger.info(f"[DB] Inserted {len(df)} rows ({prediction_type})")
